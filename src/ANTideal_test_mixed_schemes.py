...
flags.DEFINE_string('schemes', '5', 'Routing scheme.')
...
flags.DEFINE_string('ph_type', 'sum', 'How to add heuristics to pheromones')
# For future work
# flags.DEFINE_integer('mobile', 200, 'Root dir of project.')
# flags.DEFINE_bool('link_fail', False, 'Link fail and mobility.')
...
Policy universe:
    - Ant-BP:  not included here
    - Ant-BP-mirror:  not included here
    - Ant-baseline:  not included here
    - Ant-coldstart:  not included here
    - Ant-ideal (AntHocNet):
    - SP-BP:   not included here
...
output_csv = os.path.join(output_dir,
                          f'test_{datapath.split("/")[-1]}_ir_{cf_radius:.1f}_opts_{schemes_txt}_h_{ph_type}_link_{link_rate_avg}_pb_{pburst}_ls_{load_streaming}_lb_{load_bursty}_robust_{robust_test}_T_{T}_TV_{TV}.csv')
...
for id in range(len(val_mat_names)):
    ...
    for f_case in range(10):
        ...
        if FLAGS.gtype.lower() == 'poisson':
            bp_env = BackpressureAntHoc(NUM_NODES, T, seed, m, pos_c, cf_radius=cf_radius, gtype=filepath)
        else:
            bp_env = BackpressureAntHoc(NUM_NODES, T, seed, m, pos_c, cf_radius=cf_radius, gtype=FLAGS.gtype)
        ...
        for scheme in schemes:
            ...
            for vi in range(len(opts)):
                ...
                if opts[0] is not None:
                    items = 'antideal_{}_{:.3f}_{:.1f}_{:.2f}_{}_{}'.format(func, decay, exploration_rate, unit,
                                                                            not_going_back, ph_diff, ph_type)
