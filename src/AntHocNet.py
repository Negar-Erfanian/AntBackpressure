...
class BackpressureAntHoc:
    def __init__(self, num_nodes, T, seed=3, m=2, pos=None, cf_radius=0.0, gtype='ba'):
        self.num_nodes = int(num_nodes)
        self.T = int(T)
        if self.T == 10000:
            self.t_recordings = [0, 99, 199, 499, 999, 1999, 4999, 9999]
        elif self.T == 1000:
            self.t_recordings = [0, 1, 99, 199, 499, 999]
        elif self.T == 500:
            self.t_recordings = [0, 1, 99, 199, 499]
        elif self.T == 500:
            self.t_recordings = [0, 1, 99, 199, 499]
        elif self.T == 100:
            self.t_recordings = [0, 1, 99]
    ...
    # def link_failure(self, link_bias_vec):
    #     # print(f'self.link_list is {self.link_list} ')
    #     deleted_link_index = np.random.randint(0, self.num_links)
    #     self.link_list.pop(deleted_link_index)
    ...
    #     self.link_rates = np.delete(self.link_rates, deleted_link_index, axis=0)
    ...
    #     self.link_comd_cnts = np.delete(self.link_comd_cnts, deleted_link_index, axis=0)
    #     self.di_link_comd_cnts = np.delete(self.di_link_comd_cnts, deleted_link_index + self.num_links, axis=0)
    #     self.di_link_comd_cnts = np.delete(self.di_link_comd_cnts, deleted_link_index, axis=0)
    #     self.pkt_vis = np.delete(self.pkt_vis, deleted_link_index + self.num_links, axis=0)
    #     self.pkt_vis = np.delete(self.pkt_vis, deleted_link_index, axis=0)
    #     self.W = np.delete(self.W, deleted_link_index, axis=0)
    #     self.WSign = np.delete(self.WSign, deleted_link_index, axis=0)
    #     self.opt_comd_mtx = np.delete(self.opt_comd_mtx, deleted_link_index, axis=0)
    #     link_bias_vec = np.delete(link_bias_vec, deleted_link_index, axis=0)
    #     self.num_links -= 1
    #     return link_bias_vec
    ...
    def pkt_arrival(self, t, phase='proactive'):
        for fidx in range(self.num_flows):
            flow = self.flows[fidx]
            src = flow.source_node
            dst = flow.dest_node
            arrival_sofar = np.sum(self.flows_arrivals[fidx, 0:t])
            ...
            for i in range(self.flows_arrivals[fidx, t]):
                ...
                if phase == 'reactive':
                    self.backlog_ph[src][src].put((t, t, dst, [src], 'r'))
                else:
                    self.backlog_ph[src][src].put((t, t, dst, [src], 'd'))
                    arrival_sofar += 1
                    if arrival_sofar % 100 == 0:
                        self.backlog_ph[src][src].put((t, t, dst, [src], 'p'))
                        self.queue_lengths[src, src] += 1
    ...
    def reactive_sp(self, bias_matrix):
        val_max = np.amax(self.pheromones)
        self.pheromones = 0 * self.pheromones
        for flow in self.flows:
            ...
            while v != dst:
                ...
                j = np.argmin(bias_matrix[nb_set, dst])
                u = nb_set[j]
                if (v, u) in self.link_list:
                    ...
                    dlink = i + self.num_links
                else:
                    continue
                self.pheromones[dlink, dst] = val_max
                v = u

    def ph_routing(self, t, func='proportional', exploration_rate=0.0, not_going_back=False, link_bias=None,
                   ph_diff=False, alpha=5, p1=1, p2=1, ph_type='sum'):
        W_amp = np.zeros((self.num_links,), dtype=float)
        W_sign = np.ones((self.num_links,), dtype=float)
        for v in range(self.num_nodes):
            ...
            for j in range(n_undecided):
                ...
                path.append(u)
                self.backlog_ph[v][u].put((t0, t, cmd, path, role))
    ...
    def transmission_ph(self, t, mwis):
        ...
        if t == 0:
            self.pheromones_vis[..., t] = self.pheromones
        ...
        for idx in range(schs.size):
            ...
            for i in range(int(num)):
                ...
                t0, t1, cmd, path, role = pkt
                cmd_fid = self.fid_cmd_map[cmd]
                # if role == 'p' and not self.pheromone_freezed:
                #     self.pheromones[dlink, cmd] += self.phmns_unit
                if dst == cmd:
                    fidx = self.dst_nodes.index(cmd)
                    self.flows_sink_departures[fidx, t] += 1
                    cost = float(t - t0) + 1.0
                    if role != 'p':
                        self.backlog_ph[dst][self.num_nodes].put((t0, t, cmd, path, role))
                    if role != 'd':
                    # else:
                        for j in range(len(path) - 1):
                            u = path[j]
                            v = path[j + 1]
                            if (u, v) in self.link_list:
                                dlink = self.link_list.index((u, v))
                            else:
                                dlink = self.link_list.index((v, u)) + self.num_links
                            self.pheromones[dlink, cmd_fid] += 1 / cost
                            # (1- self.phmns_decay)*1e12/(lowest_cost)

                else:
                    self.backlog_ph[dst][dst].put((t0, t, cmd, path, role))
    ...
    def collect_info(self, t):
        flows_in = self.flows_arrivals[:, :t].sum(axis=1)  # packets of all flows summed across time #num_flows
        flows_out = np.zeros((self.num_flows,), dtype=int)
        for fidx in range(self.num_flows):
            flow = self.flows[fidx]
            dst = flow.dest_node
            flows_out[fidx] = len(self.backlog_ph[dst][self.num_nodes].queue)
            # print(f'flows_in is {flows_in[fidx]} and flows_out is {flows_out[fidx]}')
        # if t==self.T-1:
        #     print(f'flows_out is {flows_out} and {flows_out.all()==0}')
        # if t==self.T-1 and flows_out.all()==0:
        #     return None
        # if flows_in.all()!=0:
        #     return True
        # else:
        ...
        for fidx in range(self.num_flows):
            ...
            for i in range(flows_out[fidx]):
                if opt > 0:
                    ...
                    t0, t1, cmd, path, role = self.backlog_ph[dst][self.num_nodes].queue[i]
            ...
            for v in range(self.num_nodes):
                ...
                for u in nb_set:
                    ...
                    for idx in range(qsize):
                        t0, t1, cmd, path, role = self.backlog_ph[v][u].queue[idx]
                        cmd_fid = self.fid_cmd_map[cmd]
...
def main(opt):
    ...
    # matfile = '../data/data_poisson_10/poisson_graph_seed115_m8_n100_f17.mat'
    # bp_env = BackpressureAntHoc(NUM_NODES, T, seed, cf_radius=cf_radius, gtype=matfile)
    bp_env = BackpressureAntHoc(NUM_NODES, T, seed, cf_radius=cf_radius, gtype='ba')
    ...
    if phase != 'reactive':
        bp_env.load_pherom(opt, case_name, 0, 'test_anthoc', T - 1, 'stream')
    ...
    # bp_env.reactive_sp(bias_matrix)

    for t in range(bp_env.T):
        bp_env.pkt_arrival(t, phase=phase)
    ...
    if burst_prob == 0.0 and phase == 'reactive':
        bp_env.save_pherom(opt, case_name, 0, 'test_anthoc', 'stream')

    logfile = os.path.join("..", "log", f"debug_decay_{decay}_explorationrate_{exploration_rate}_function_{func}_num_nodes_{NUM_NODES}_time_{T}_anthoc.txt")
    with open(logfile, "a") as f:
        print("\nph_diff is {}".format(ph_diff), file=f)
        print("phase {}".format(phase), file=f)
